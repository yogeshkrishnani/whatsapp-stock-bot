...
app.post('/webhook', (req, res) => {
    ...
    console.log(`📊 User Activity: ${profileName || 'Unknown'} (${fromNumber}) - "${messageBody}"`);
...
async function processStockRequest(messageBody, fromNumber) {
    try {
        ...
        if (!stockNames) {
            ...
            await sendWhatsAppMessage('कृपया स्टॉक का नाम भेजें। जैसे: TCS या Reliance', fromNumber);
            return;
        }

        // Send acknowledgment message
        await sendWhatsAppMessage('📊 विश्लेषण कर रहे हैं... कृपया 30 सेकंड रुकें', fromNumber);
        ...
        if (error.message && error.message.includes('API')) {
            errorMessage = 'स्टॉक डेटा प्राप्त करने में समस्या हुई। कृपया बाद में कोशिश करें।';
...
async function sendWhatsAppMessage(messageText, toNumber = TARGET_PHONE_NUMBER) {
    try {
        console.log(`📤 Sending to ${toNumber}:`, messageText.substring(0, 50) + '...');
...
app.listen(port, () => {
    ...
    console.log(`• TARGET_PHONE_NUMBER: ${process.env.TARGET_PHONE_NUMBER ? '✅ Set' : '❌ Missing'}`);
